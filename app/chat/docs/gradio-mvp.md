...
UI는 내부 구조를 알지 않는다. 오직 `ChatRequest`를 만들고 `run_chat`을 호출한 뒤 `ChatResponse`를 표시한다.
...
- 검색 문서 수 top_k 선택
- LLM 답변 사용 여부 선택
- graph/pipeline engine 선택
- 답변 출력
- 근거 citation 출력
- route, retry, error metadata 출력
...
- full tests: 76 passed, 1 warning
- ruff: pass
- compileall: pass
